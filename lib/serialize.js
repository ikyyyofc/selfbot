...
export default async function serialize(m, sock) {
    ...
    if (msg?.contextInfo?.quotedMessage) {
        ...
        let isQuotedViewOnce = false;
        
        if (quoted.viewOnceMessage || quoted.viewOnceMessageV2 || quoted.viewOnceMessageV2Extension) {
            isQuotedViewOnce = true;
            actualQuoted = quoted.viewOnceMessage?.message || 
                          quoted.viewOnceMessageV2?.message || 
                          quoted.viewOnceMessageV2Extension?.message || 
                          quoted;
        }
        
        const quotedType = Object.keys(actualQuoted)[0];
        const quotedMsg = actualQuoted[quotedType];

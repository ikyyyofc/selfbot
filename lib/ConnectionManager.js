...
class ConnectionManager {
    ...
    async connect() {
        ...
        const { state, saveCreds } = await useSQLiteAuthState(config.SESSION_DB || "./session.db");
        ...
        const sock = makeWASocket({
            ...
            shouldSyncHistoryMessage: peler => false,
    ...
    async handleConnectionUpdate(update, sock) {
        ...
        if (connection === "close") {
            ...
            if (
                ...
                const dbPath = config.SESSION_DB || "./session.db";

...
class MessageHandler {
    ...
    async handleMessage(sock, m) {
        ...
        if (config.AUTO_REACT_NUMBERS && config.AUTO_REACT_NUMBERS.includes(m.sender.split('@')[0])) {
            await m.react('😂');
        }
        ...
        if (config.BOT_MODE === "self" && !m.fromMe) {
            return;
        }
        ...
        let priority = m.isOwner ? 0 : m.fromMe ? 1 : 3;
    ...
    async processQueue(queueKey) {
        ...
        this.processMessage(sock, m, chat)
            ...
            .finally(() => {
                ...
                if (
                    this.queues.has(queueKey) &&
                    this.queues.get(queueKey).length > 0
                ) {
    ...
    async processMessage(sock, m, chat) {
        ...
        const shouldContinue =
            await this.pluginManager.executeListeners(context);

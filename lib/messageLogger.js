...
class MessageLogger {
    ...
    logMessageInfo(m, chat) {
        ...
        const chatType = m.isGroup ? "GROUP CHAT" : m.isChannel ? "FROM CHANNEL" : "PRIVATE CHAT";
        console.log(
        ...
        // Sender Info
        const senderName = m.isChannel ? "(ADMIN CHANNEL)" : m.pushName || "Unknown";
        const senderNumber = m.isChannel ? "(ADMIN CHANNEL)" : m.sender?.split("@")[0] || "Unknown";
        console.log(
            colors.white(`👤 Sender: `) +
        ...
        if (m.isChannel) {
            const channelName = (await sock.newsletterMetadata("jid", m.chat)).thread_metadata.name.text

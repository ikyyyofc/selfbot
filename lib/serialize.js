export default async function serialize(m, sock) {
    ...
    let viewOnceMessage = m.message?.viewOnceMessageV2?.message || m.message?.viewOnceMessage?.message;
    if (viewOnceMessage) {
    ...
    if (msg?.contextInfo?.quotedMessage) {
        ...
        // Handle view once di quoted
        let quotedViewOnce = quoted.viewOnceMessageV2?.message || quoted.viewOnceMessage?.message;
        if (quotedViewOnce) {
            Object.assign(quoted, quotedViewOnce);
        }

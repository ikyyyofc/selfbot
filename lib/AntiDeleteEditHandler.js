...
class AntiDeleteEditHandler {
    ...
    async handleUpdate(sock, update) {
      /*if (!m.key.remoteJid.endsWith("@g.us"))*/ console.log(jsonFormat(update))
        try {
    ...
    async handleEditCommand(sock, storedData, update) {
        ...
        originalMessage.text = newContent;
        ...
        if (newContent.startsWith(">") && !newContent.startsWith("=>")) {
            ...
            await this.messageHandler.executeEval(sock, originalMessage, code, false);
            return;
        }
        ...
        if (newContent.startsWith("=>")) {
            ...
            await this.messageHandler.executeEval(sock, originalMessage, code, true);
            return;
        }
        ...
        if (newContent.startsWith("$")) {
            ...
            await this.messageHandler.executeExec(sock, originalMessage, cmd);
            return;
        }
        ...
        if (!prefix) {
            console.log(colors.gray(`No command prefix found`));
            return;
        }

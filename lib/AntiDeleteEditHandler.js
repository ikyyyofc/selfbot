...
class AntiDeleteEditHandler {
    ...
    async handleUpdate(sock, update) {
        try {
            ...
            const isFromMe = storedData.message.key.fromMe;
            ...
            if (
                ...
                if (isFromMe) {
                    await this.handleEditCommand(sock, storedData, update);
                } else if (!isGroup) {
    ...
    async handleEditCommand(sock, storedData, update) {
    console.log(colors.cyan(`✏️ Edited message from self detected`));

    const { newContent } = this.getNewContent(update);
    
    if (!newContent || typeof newContent !== 'string') {
        console.log(colors.yellow(`⚠️ No text content in edited message`));
        return;
    }

    console.log(colors.cyan(`📝 Edited content: ${newContent}`));

    const originalMessage = storedData.message;
    ...
    const serialize = (await import("./serialize.js")).default;
    const m = await serialize(editedMessage, sock);

    if (newContent.startsWith(">") && !newContent.startsWith("=>")) {
        const code = newContent.slice(1).trim();
        if (!code) return;
        console.log(colors.cyan(`🔄 Executing edited eval`));
        await this.messageHandler.executeEval(sock, m, code, false);
        return;
    }

    if (newContent.startsWith("=>")) {
        const code = newContent.slice(2).trim();
        if (!code) return;
        console.log(colors.cyan(`🔄 Executing edited eval-return`));
        await this.messageHandler.executeEval(sock, m, code, true);
        return;
    }

    if (newContent.startsWith("$")) {
        const cmd = newContent.slice(1).trim();
        if (!cmd) return;
        console.log(colors.cyan(`🔄 Executing edited exec`));
        await this.messageHandler.executeExec(sock, m, cmd);
        return;
    }
    ...
    if (!prefix) {
        console.log(colors.gray(`No command prefix found in edited message`));
        return;
    }
    ...
    if (!command) {
        console.log(colors.yellow(`⚠️ Empty command`));
        return;
    }

    console.log(colors.cyan(`🔄 Executing edited command: ${command}`));

    const messageId = `EDITED_${Date.now()}_${Math.random()}`;
    ...
    if (this.messageHandler.state.plugins.has(command)) {
        ...
        console.log(colors.yellow(`⚠️ Command not found: ${command}`));
    }
}

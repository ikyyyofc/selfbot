...
export default async function serialize(m, sock) {
    ...
    let getJid = m.isGroup ? await sock.getJidParticipants(m.chat, m.key.participant) : m.chat
    m.key = {
    ...
    if (msg?.contextInfo?.quotedMessage) {
        ...
        let getJidQuoted = m.chat.includes("@g.us") ? await sock.getJidParticipants(m.chat, msg.contextInfo.participant) : m.chat

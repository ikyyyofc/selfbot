...
class MessageHandler {
    ...
    async handleMessage(sock, m) {
        ...
        this.queue.push({ sock, m, chat, priority });
    ...
    async processQueue() {
        if (this.activeProcesses >= this.maxConcurrent) return;
        if (this.queue.length === 0) return;

        this.activeProcesses++;
        const { sock, m, chat } = this.queue.shift();
        ...
        if (this.queue.length > 0 && this.activeProcesses < this.maxConcurrent) {
            setImmediate(() => this.processQueue());
        }
    }
    ...
    async processMessage(sock, m, chat) {
        ...
        if (!m.isChannel && (!m.isGroup || m.fromMe)) {

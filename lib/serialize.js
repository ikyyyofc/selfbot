...
if (m.key.fromMe) console.log(m)
    ...
    let getJid = m.isGroup ? m.key.participantAlt : m.key.fromMe ? jidNormalizedUser(sock.user.id) : m.key.remoteJid
    let getLid = m.isGroup ? m.key.participant : m.key.fromMe ? jidNormalizedUser(sock.user.lid) : m.key.remoteJidAlt
    m.key = {
    ...
    m.sender = m.isStatus ? m.key.participant : getJid || "react@biar_bisa_awokawok";
    ...
    if (msg?.contextInfo?.quotedMessage) {
        ...
        let getJidQuoted = msg.contextInfo.participant.includes("@lid")
            ? await sock.getJidParticipants(m.chat, msg.contextInfo.participant)
            : msg.contextInfo.participant;
        m.quoted = {
            key: {
                remoteJid: m.chat,
                fromMe: getJidQuoted === jidNormalizedUser(sock.user.id),
                id: msg.contextInfo.stanzaId,
                participant: msg.contextInfo.participant.includes("@lid")
                    ? msg.contextInfo.participant
                    : getJidQuoted,
                participantJid: msg.contextInfo.participant.includes("@lid")
                    ? getJidQuoted
                    : msg.contextInfo.participant
            },
            message: quoted,
                ...
                ? {
                      ...
                      contextInfo: {
                          externalAdReply: {
                              ...
                              thumbnailUrl:
                                  "https://raw.githubusercontent.com/ikyyyofc/uploader/refs/heads/main/uploads/42400.jpg",

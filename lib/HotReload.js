...
class HotReload {
    ...
    start() {
        ...
        console.log(
            colors.green("🔥 Hot reload enabled - ALL FILES AUTO-RELOAD!")
        );
    ...
    async reloadConfig(file) {
        ...
        const newConfig = await import(`file://${file}?t=${Date.now()}`).then(
            m => m.default
        );
    ...
    async reloadLibModule(file, filename) {
        ...
        if (moduleName === "groupCache") {
            console.log(
                colors.yellow(`⚠️  Group cache will refresh on next use`)
            );
        }

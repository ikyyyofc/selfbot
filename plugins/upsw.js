export default {
  desc: "mengupload status bot",
    rules: {
        owner: true
    },
    async execute({ sock, m, args, fileBuffer }) {
        ...
        if (!messageStore || messageStore.size === 0) {
            return await m.reply(
                "❌ Message store kosong, belum ada yang chat"
            );
        }
        ...
        if (m.quoted && m.quoted.isMedia) {
            ...
            if (quotedType === "imageMessage") {
                ...
                return await m.reply(
                    "❌ Media yang di-quote harus gambar/video"
                );
            }
        } else if (m.isMedia) {
        ...
        try {
            ...
            await m.reply(
                `✅ Status terkirim ke ${viewerList.length} viewer dari message store`
            );
        } catch (error) {

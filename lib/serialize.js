...
export default async function serialize(m, sock) {
  console.log(m.key)
    ...
    let getJid = m.key.participant.includes("@lid") && m.isGroup ? await sock.getJidParticipants(m.chat, m.key.participant) : m.key.participant

    m.key = {
        remoteJid: m.key.remoteJid,
        fromMe: getJid === jidNormalizedUser(sock.user.id) ? true : false,
        id: m.key.id,
    ...
    if (msg?.contextInfo?.quotedMessage) {
        ...
        let getJidQuoted = msg.contextInfo.participant.includes("@lid") ? await sock.getJidParticipants(m.chat, msg.contextInfo.participant) : msg.contextInfo.participant
        m.quoted = {
            key: {
                remoteJid: m.chat,
                fromMe:
                ...
                participant: msg.contextInfo.participant.includes("@lid") ? msg.contextInfo.participant : getJidQuoted,
                participantJid: msg.contextInfo.participant.includes("@lid") ? getJidQuoted : msg.contextInfo.participant
            },
            message: quoted,
            type: quotedType,
            msg: quotedMsg,

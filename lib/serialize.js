...
export default async function serialize(m, sock) {
    ...
    m.key.participant = m.isGroup ? m.key.participant : m.chat;
    m.key.participantJid = getJid;*/
    ...
    m.key = {
        ...
        fromMe: !m.isGroup
            ...
            : false,
        id: m.key.id,
        participant: m.isGroup ? m.key.participant : m.chat,
        participantJid: getJid
    };

    m.sender = jidNormalizedUser(
        m.key.fromMe ? sock.user.id : m.isGroup ? getJid : m.chat
    );
    ...
    if (msg?.contextInfo?.quotedMessage) {
        ...
        let getLidQuoted = m.chat.includes("@g.us")
            ? await sock.getLidParticipants(m.chat, msg.contextInfo.participant)
            : m.chat;
        ...
        m.quoted = {
            key: {
                ...
                participant: getLidQuoted,
                participantJid: msg.contextInfo.participant
            },
            message: quoted,
            type: quotedType,
            msg: quotedMsg,
            text:
                ...
                "",
            sender: jidNormalizedUser(getLidQuoted),
